...
regressor_path = root_path / "models/model.joblib"
...
if date and time:
       ...
       next_interval = next_interval.replace(
              minute=(next_interval.minute // 15) * 15,
              second=0,
              microsecond=0,
       )
       if next_interval.date() != date:
              st.error("Please select a time before 11:45 PM.")
              st.stop()
